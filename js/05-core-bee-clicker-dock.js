/* Study Hive  -  file 05: core-bee-clicker-dock
...
(function(){
  ...
  /* ---------------- 7. Bee-clicker mini-game (double-click the hive) ---------------- */
  ...
  /* REFINEMENT: the Bee Clicker game used to open on DOUBLE-CLICK of the hive
     icon. That conflicted with the single-click motivation bubble, fired by
     accident, and fought the phone browser's double-tap-to-zoom (the "glitch"
     every time). The game now opens from a clean, deliberate button in the
     Hive Controls menu instead. Single-click on the hive = motivation only. */
  function addClickerLauncher(){
    var panel = $('hiveMenuPanel');
    if (!panel || $('openClickerBtn')) return;
    panel.insertAdjacentHTML('beforeend', '<button id="openClickerBtn">🎮 Bee Clicker</button>');
    $('openClickerBtn').addEventListener('click', function(e){
  ...
  addClickerLauncher();
  setInterval(addClickerLauncher, 4000);
  ...
  var SECRETS_LIST = [
    ...
    { id: 'clicker',  icon: '🎮', name: 'Hive Reflexes',   hint: 'Open 🎮 Bee Clicker from the Hive Controls (☰ Hive) menu' },
...
(function () {
 ...
 ORDER MATTERS: files run in numeric-prefix order.
*/
...
(function(){
  ...
  if (clickerCloseBtn) clickerCloseBtn.addEventListener('click', endClicker);
  var hiveWrapEl = $('hiveWrap');
  if (hiveWrapEl) hiveWrapEl.addEventListener('dblclick', function(e){ e.stopPropagation(); openClicker(); });
